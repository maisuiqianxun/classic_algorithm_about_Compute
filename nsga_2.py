...
def f_x2(x2):
    return (x2-2)*(x2-2)
def fast_non_dominated_sort(N, l, rank_p,length_Fy,R_decode):   # N records half of the all population of merged_Population R
...
def crowd_distance_assignment( L,num_solu,R_decode):
    ...
    for i in range(1,3):   # for each objective m
        ...
        for j in range(1,num_solu-1):
            distance_L[j,i]=(distance_L[j+1,i]-distance_L[j-1,i])/(f_max-f_min)
...
for genera in range(generation):
    ...
    F=fast_non_dominated_sort(N,l,rank_p,length_Fy,R_decode)
    sum_first2i=0
    ...
    if( k<N ):
        ...
        guide_Parent_new[k:N-1]=Len[length_Fy[j]-(N-k) ,0]  # copy elements from end to begin because of ascending "Len"
    guide_Parent_new=guide_Parent_new.reshape(200)
    Parent=R[guide_Parent_new,:]
    Offspring=make_new_pop(Parent)
    print("the generation is %d th\n",genera )
